...
export default function PartsPage() {
  ...
  const load = () => {
    ...
    return fetch(`/api/stash/parts${qs ? '?' + qs : ''}`).then(r => r.json()).then(setParts).finally(() => setLoading(false));
  };

  useEffect(() => {
    load();
    fetch('/api/stash/parts/classifications').then(r => r.json()).then(setClassifications);
  }, []);
