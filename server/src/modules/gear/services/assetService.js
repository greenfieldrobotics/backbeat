...
export async function createAsset(db, {
  ...
  if (location_id) {
    const { rows } = await db.query('SELECT id FROM locations WHERE id = $1', [location_id]);
...
export async function updateAsset(db, id, changes) {
  ...
  const { rows: existingRows } = await db.query('SELECT * FROM assets WHERE id = $1', [id]);
  ...
  if (locId) {
    const { rows: locRows } = await db.query('SELECT id FROM locations WHERE id = $1', [locId]);
