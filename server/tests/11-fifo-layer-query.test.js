...
describe('FIFO Layer Query Endpoint', () => {
  ...
  test('Default returns only active layers (remaining_qty > 0)', async () => {
    ...
    const res = await request(app).get('/api/stash/inventory/fifo-layers');
  ...
  test('include_depleted=true returns all layers', async () => {
    ...
    const res = await request(app).get('/api/stash/inventory/fifo-layers?include_depleted=true');
    expect(res.status).toBe(200);
  ...
  test('Filter by part_id', async () => {
    ...
    const res = await request(app).get(`/api/stash/inventory/fifo-layers?part_id=${partA.id}`);
    expect(res.status).toBe(200);
  ...
  test('Filter by location_id', async () => {
    ...
    const res = await request(app).get(`/api/stash/inventory/fifo-layers?location_id=${loc1.id}`);
    expect(res.status).toBe(200);
  ...
  test('Combined part_id + location_id filter', async () => {
    ...
    const res = await request(app).get(`/api/stash/inventory/fifo-layers?part_id=${partA.id}&location_id=${loc1.id}`);
    expect(res.status).toBe(200);
    expect(res.body.length).toBe(1);
